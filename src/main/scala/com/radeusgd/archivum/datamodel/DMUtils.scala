...
object DMUtils {
   def parsePath(path: String): Seq[String] = path.split(".")

   // TODO not sure if take DMValue or DMStruct
   def makeGetter(path: Seq[String]): (DMAggregate) => DMValue =
      path match {
   ...
   def makeSetter(path: Seq[String]): (DMStruct, DMValue) => DMStruct =
      path match {
         case Nil => throw new IllegalArgumentException // TODO NonEmptySeq
