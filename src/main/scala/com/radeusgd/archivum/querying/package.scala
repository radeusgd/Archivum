...
package object querying {
    ...
    (v: DMValue) =>
      {
        try {
          ...
          struct.values.getOrElse(
            key,
            if (safe) DMNull else throw new NoSuchElementException(key)
          )
        } catch {
  ...
  implicit class DMValueHelper(val dmv: DMValue) extends AnyVal {
    ...
    def taknieasBool: Option[Boolean] =
      dmv match {
  ...
  implicit class NestedMapOfSequences[K, T](val nm: NestedMapADT[K, Seq[T]])
      extends AnyVal {
    def count: Int =
      nm match {
  ...
  def percentage(part: Int, whole: Int): DMValue =
    DMStruct(
      Map("part" -> DMInteger(part), "whole" -> DMInteger(whole))
    ) // this is a special encoding which XLS exporter understands
  ...
  // This does not act as one would expect
...
//   }
  ...
  implicit class PathGetterHelper(val getter: DMValue => DMValue)
      extends AnyVal {
