...
case class Query(
    ...
    realization: ResultSet => Seq[ResultRow]
) {
...
abstract class BuiltinQuery(
  ...
  def prepareTask(resultPath: Path, repo: Repository): Task[Unit] =
    new Task[Unit]() {

      val charakterFilter: SearchCriteria =
        charakter
          .map(ch => Equal(Seq("Charakter miejscowości"), DMString(ch)))
          .getOrElse(Truth)
      ...
      private def runManualQueries(workToDo: Int): Unit = {
        for (((qname, query), index) <- manualQueries.zipWithIndex) {
          ...
          XLSExport.export(resultPath.resolve(qname + fileExt), res)
      ...
      override def call(): Unit = {
        val classicQueries: Map[String, Either[ClassicQuery, Query]] =
          ungroupedQueries.mapValues(Left(_)) ++ groupedQueries.mapValues(
            Right(_)
          )
        ...
        if (folderGroupings.isEmpty) {
          ...
          val firstLevelGroupings = repo.getAllDistinctValues(
            firstLevelGroupPath,
            filter = charakterFilter
          )
          val flgLen = firstLevelGroupings.length
          val workToDo =
            classicQueries.size * (flgLen + 1) + 1 + manualQueries.size
          ...
          for ((qname, queryDesc) <- classicQueries) {
            ...
            def runQuery(
              ...
              if (restGroupings.isEmpty) {
                ...
                val path = resultPath.resolve(subName).resolve(qname + ".xlsx")
            ...
            for (firstLevelGroupValue <- firstLevelGroupings) {
              if (firstLevelGroupValue.toString.isEmpty) {
                println(
                  s"Skipping some records because their ${firstLevelGroupPath.toString()} is empty"
                )
              } else {
                ...
                runQuery(
                  ...
                  firstLevelGroupValue.toString,
                  folderGroupings.tail
                )
              }
              progress += 1
              updateProgress(progress, workToDo)
