...
class Zgony(
  ...
  private def przyczynyZgonówPionowo(rs: ResultSet): Seq[ResultRow] =
    rs.groupBy(GroupBy("Przyczyna zgonu", PopularitySorted(Descending)))
      .aggregateClassic("Liczba" -> ClassicAggregations.count)

  override val groupedQueries: Map[String, Query] = Map(
    "Sezonowość tygodniowa pogrzebów" -> Query(
      DataPochówku,
      (rs: ResultSet) => rs.countHorizontal(groupByWeekday("Data pochówku"))
    ),
    "Najczęstsze przyczyny zgonów" -> Query(
      DataŚmierci,
      przyczynyZgonówPionowo
    ),
    "Przyczyny zgonów" -> Query(DataŚmierci, przyczynyZgonówPoziomo),
    "Liczba dni od zgonu do pochówku" -> Query(
      DataPochówku,
      liczbaDniOdŚmierciDoPochówku
    ),
    "Sezonowość miesięczna zgonów" -> Query(
      DataŚmierci,
      (rs: ResultSet) => rs.countHorizontal(groupByMonth("Data śmierci"))
    ),
    "Zgony rocznie" -> Query(
      DataŚmierci,
      (rs: ResultSet) =>
