...
case class ListMap[K, V] private (private val elems: List[(K, V)]) {
  ...
  def mapValues[U](f: V => U): ListMap[K, U] =
    new ListMap[K, U](
...
object ListMap {
  ...
  def fromList[K, V](entries: List[(K, V)]): ListMap[K, V] =
    new ListMap[K, V](entries.reverse)
}
...
object NestedMapADT {
  ...
  def merge[K, V](a: NestedMapADT[K, V], b: NestedMapADT[K, V])(implicit
      merger: Semigroup[V]
  ): NestedMapADT[K, V] = {
    (a, b) match {
      case (NestedMapElement(va), NestedMapElement(vb)) =>
        NestedMapElement(merger.combine(va, vb))
      case (NestedMap(ma), NestedMap(mb)) =>
        if (ma.keys != mb.keys)
          throw new RuntimeException("NestedMapADT.merge - incompatible shapes")
        else {
...
case class NestedMapElement[K, +V](value: V) extends NestedMapADT[K, V] {
  ...
  override def flatMap1[U](f: V => NestedMapADT[K, U]): NestedMapADT[K, U] =
    f(value)
...
case class NestedMap[K, V](mapping: ListMap[K, NestedMapADT[K, V]])
    extends NestedMapADT[K, V] {

  def withHeader(header: K): NestedMap[K, V] =
    NestedMap.fromSubMaps(header -> this)
  ...
  override def asSingleton: NestedMapElement[K, V] =
    throw new RuntimeException("Not a singleton NestedMap")

  def append(other: NestedMap[K, V]): NestedMap[K, V] =
    NestedMap(mapping.append(other.mapping))

  override def toString: String =
    "NestedMap(" + mapping.entries
      .map({ case (k, v) => k + "->" + v })
      .mkString(", ") + ")"
}
...
object NestedMap {
  ...
  def fromSubMaps[K, V](mappings: (K, NestedMapADT[K, V])*): NestedMap[K, V] =
    new NestedMap(ListMap.fromList(mappings.toList))
}
