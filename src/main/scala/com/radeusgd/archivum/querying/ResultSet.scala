...
case class ResultSet(rows: Seq[MultipleResultRow]) {
  ...
  def filterGroups(
      predicate: NestedMapADT[String, Seq[DMValue]] => Boolean
  ): ResultSet =
  ...
  def deepMap(f: DMValue => DMValue): ResultSet =
    ResultSet(
      rows.map(mrr => MultipleResultRow(mrr.prefix, mrr.objects.map(_.map(f))))
    )
  ...
  def deepFlatMap(f: DMValue => Seq[DMValue]): ResultSet = {
    ResultSet(
      rows.map(mrr =>
        MultipleResultRow(mrr.prefix, mrr.objects.map(_.flatMap(f)))
      )
    )
  }
  ...
  private def computeHorizontalGroupingPreset(
      grouping: Grouping
  ): Seq[String] = {
    ...
    val preset: Seq[String] =
      ALLGROUPED.extractFirsts.map(Grouping.groupkeyToString)
    preset
  }
  ...
  def aggregateClassic(
      aggregations: (String, ClassicAggregations.Aggregation)*
  ): Seq[ResultRow] = {
  ...
  def countGroups(
    ...
    groupBy(
      GroupBy(
        path,
        PopularitySorted(Descending),
        CustomAppendColumn(nameColumn)
      )
    ).aggregateClassic(
  ...
  def countTransposed(
      path: String,
      traits: Seq[(String, DMValue)],
      default: Option[String]
  ): Seq[ResultRow] =
    aggregateClassic(
      ClassicAggregations.countTransposed(path, traits, default): _*
    )

  def countHorizontal(
      grouping: Grouping,
      includePercentages: Boolean = false,
      appendHeader: Option[String] = None
  ): Seq[ResultRow] = {
    val preset = computeHorizontalGroupingPreset(grouping)
    rows.map(row =>
      row.countHorizontal(grouping, preset, includePercentages, appendHeader)
    )
  }

  def countAfterGrouping(columnName: String = "l.b."): Seq[ResultRow] = {
    rows.map(
      _.aggregate(objs => ResultRow(columnName -> DMInteger(objs.length)))
    )
  }
  ...
  def ungroup(): ResultSet = {
    ...
    val merged = objs.reduce(
      NestedMapADT.merge(_, _)((x: Seq[DMValue], y: Seq[DMValue]) => x ++ y)
    )
    ResultSet(Seq(MultipleResultRow(ResultRow.empty, merged)))
  }

  override def toString: String =
    "ResultSet(with " + rows.length + " MultiRows)"
